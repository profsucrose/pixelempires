...
public class GiveCoins implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        try {
            Economy.add(uuidString, coinAmount);
        } catch (Exception e) {
        ...
        player.sendMessage(ChatColor.GOLD + "Gave coins successfully");
        if (coinAmount > 0) {
            receiverPlayer.sendMessage(ChatColor.GOLD + "You have been gifted " + coinAmount + " Pixel Coins from the gods!");
        } else {
